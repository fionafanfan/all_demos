...
class Logger(object):
    ...
    def __init__(self):
        ...
        base_io_path = 'log'
        if not os.path.isdir(base_io_path):
            os.makedirs(base_io_path)

        today = datetime.now().strftime('%Y%m%d')
        today_io_path = '{}{}'.format(base_io_path,today)
        io_path = os.path.join(base_io_path, today_io_path)
        if not os.path.exists(io_path):
            os.makedirs(io_path)

        io_file = os.path.join(io_path, 'io.log')
        error_file = os.path.join(io_path, 'error.log')
        intent_file = os.path.join(io_path, 'intent.log')

        # io log
        self.iolog = logging.getLogger("semIo")
        self.iolog.setLevel(logging.INFO)
        # "log/io.log"
        file_outer = logging.handlers.RotatingFileHandler(io_file, mode='a', maxBytes=1024 * 1024 * 100,
        ...
        file_outer = logging.handlers.RotatingFileHandler(error_file, mode='a', maxBytes=1024 * 1024 * 100,
                                                          backupCount=10000, encoding="utf-8")
        ...
        file_outer = logging.handlers.RotatingFileHandler(intent_file, mode='a', maxBytes=1024 * 1024 * 100,
                                                          backupCount=10000, encoding="utf-8")
        file_outer.setLevel(logging.INFO)
        file_outer.setFormatter(logging.Formatter('%(asctime)s-%(levelname)s-%(threadName)s-%(message)s'))
