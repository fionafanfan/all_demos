  ...
  "database": {
        "mysql": {
            "us_stock_define": {
                ...
                "port": 3306,
                "user": "vv-dev",
                "password": "aAbl*\u0026Ln3332n",
                "database": "vv_finance_stock",
                "__table__": [
                    "t_us_stock_define"
                ]
            },
            ...
            "quotation": {
                ...
                "port": 3306,
                "user": "vv-finance",
                "password": "lk2K2ABnN2qu",
                "database": "vv_us_quotation_composite",
                "__table__": [
                    "t_stock_daily_forward_kline_117"
                ]
            },
        }
    }
...
def search_stock_skill_data():
    """
    城中性对比和投资回报对比都是取能力指标这个表里的数据，可以共一个数据查询
    """
    db = mysql('us_stock_define')
    query_sql = f"select *  from t_us_f10_key_figures where code in (select code from t_us_stock_define where industry_code=(SELECT industry_code from t_us_stock_define where code='TSLA.us'))"
    db.execute(query_sql)
    data = db.fetchall()
    return data
...
def search_stock_zongshizhi():
    ...
    """
    db = mysql('usuts')
    query_sql = f"select * from stk2301 where FIU_ID=(select FIU_ID from sym1101  where SYMBOL='TSLA.us')"
    db.execute(query_sql)
    data = db.fetchall()
    return data
...
def search_stock_table_id():
    ...
    for table_id in range(0, 128):
        print(f'开始计算:{table_id}')
        st = time.time()
        query_sql = f"select DISTINCT code from t_stock_daily_forward_kline_{table_id}"
        db.execute(query_sql)
        data = db.fetchall()
        stock_table_id_map.update({d['code']: table_id for d in data})
        print(f'{table_id}耗时:{time.time()- st}秒, 股票数量:{len(data)}')

    with open('stock_table_id_map.json', 'w') as fw:
        json.dump(stock_table_id_map, fw, indent=4)
    print(f"写入ok, 耗时:{time.time() - global_st}秒, 股票总数量:{len(stock_table_id_map)}")
    return stock_table_id_map


if __name__ == "__main__":
    mysql_mgr.init()
    # search_stock_codes()
    # search_stock_skill_data()
    # search_stock_zongshizhi()
    search_stock_table_id()
